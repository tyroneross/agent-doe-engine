...
# multi-goal — Planning Phase + Factor Adjustability + Worktree Separation + Research Seam

Status: local branch `feat/planning-phase` in worktree `~/dev/git-folder/multi-goal-planning`. **Not pushed. Not merged.** For review.

## What this adds

Four changes, composed:

1. **Phase 0 (PLAN)** — explicit new phase in `skills/multi-goal/SKILL.md` BEFORE SETUP. Walks scan → host-LLM pick → validate → optional research → write factors.json. Phase 1 is now SETUP-of-objectives only; factors come pre-validated from Phase 0.
...
3. **`scripts/worktree.py`** (new, stdlib-only, SPDX-headed, 16 tests). First-class git-worktree separation: every multi-goal run lives in `<repo>-multigoal-<slug>` on branch `multigoal/<slug>`. Subcommands `init` / `info` / `cleanup [--delete-branch]`. Idempotent init, symlink-aware path resolution.
...
```
User: /multi-goal "make build faster without blowing up bundle size"
  │
  ├─ Phase 0.0: worktree.py init --target "build-faster-bundle"
  │     → cd into <repo>-multigoal-build-faster-bundle/
  │
  ...
  │
  ├─ Phase 0.5: write .multi-goal/optimize/factors.json
  │
...
| Path | Change |
|---|---|
| `skills/multi-goal/SKILL.md` | New §Phase 0 (PLAN) with subsections 0.0–0.5; §1.2 now points at Phase 0 as canonical; §Phase 3 Review now ends with worktree cleanup |
| `commands/multi-goal.md` | Three-phase → four-phase flow; calls out Phase 0 + worktree-first |
...
Once atomize-ai's eval harness lands, run multi-goal against it as follows:

1. From atomize-ai's project root: `python3 ${MULTI_GOAL_ROOT}/scripts/worktree.py --workdir . --target "eval-quality-and-cost" init`
2. `cd ../atomize-ai-multigoal-eval-quality-and-cost`
...
Stage A is the eval harness providing the `metric_cmd` interface; Stage B (the research seam) is shipped here; Stage 0 (Phase 0 Planning) is shipped here. The whole loop is wire-compatible the day the eval harness ships, because the integration surface is the existing `metric_cmd` string — multi-goal does not need to know anything about how the eval harness runs internally.

## Notes for the reviewer

- **OAR (optimization-auto-research-plugin) untouched**, per the ask. The retirement cleanup is a separate small change.
- **No push, no merge, no marketplace publish.** Worktree is at `~/dev/git-folder/multi-goal-planning` on branch `feat/planning-phase` for direct inspection. To dispose: `git -C ~/dev/git-folder/multi-goal worktree remove ../multi-goal-planning && git -C ~/dev/git-folder/multi-goal branch -D feat/planning-phase`. To land: regular review / merge / push from the user's primary checkout.
