...
# multi-goal — cross-host guide

This file is the host-neutral version of the workflow for non-Claude coding agents (Codex, Copilot, Cursor, …). Claude Code uses `skills/multi-goal/SKILL.md`; the method is identical.
...
3. **Run.** For each row in the design (randomized order): apply the factor values, measure every objective via its `metric_cmd`, run the guard, record `{run_id, values:{...}, guard_ok}` to `.multi-goal/optimize/results.jsonl`, then revert (each DOE run starts from the same baseline).
4. **Analyze.** `python3 scripts/doe.py analyze --design doe.json --results results.jsonl --objectives objectives.json` → ranked effects per objective + the best run by the chosen selection method. Apply the winning combination as one commit.
...
- `desirability`: Derringer-Suich — transform each response to dᵢ∈[0,1], combine as D = (∏ dᵢ^wᵢ)^(1/Σw); pick max D.
