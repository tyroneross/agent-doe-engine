---
name: feedback
description: Report a bug or send feedback about the agent-doe-engine plugin
---

# Report agent-doe-engine feedback

File the user's report as a GitHub issue on `tyroneross/agent-doe-engine`. Issues are this plugin's
...
```bash
gh issue create --repo tyroneross/agent-doe-engine \
...
5. If `gh` is missing or unauthenticated, do not fail — print the URL so the user
   can open it in a browser: https://github.com/tyroneross/agent-doe-engine/issues/new
